...
- 初回0.03・約0.5秒の試験を2回実施し、異常なしを確認したが振動は知覚できなかった。
- 既定の初回上限0.03は維持した。
- `hardware_initial_test_passed=true`を明示した場合だけ0.05まで許可する第二段階gateを追加した。
- 初回確認の有無にかかわらず、Phase 4では0.05を超えるhardware設定を起動拒否する。

実機波形比較結果（2026-09-09）:

- 正弦波0.05・約0.5秒は、adapter上では正常にapply・stopしたが操作者が振動を知覚できなかった。
- 矩形波0.05・約0.5秒は2回とも知覚でき、CLEARとshutdownで正常停止した。
- 急回転、異音、発熱、Autocenter異常は確認されなかった。
- 矩形波0.05は知覚可能だが、危険通知としては弱いという評価だった。
- 実機結果に基づき、hardware backendの全active patternを`FF_SQUARE`へ変更した。
- 矩形波反映後の対象テスト65件、flake8、pydocstyle、diff check、2 packageのclean buildがPASSした。
- 強度上限は0.05のまま維持しており、Phase 4は部分完了とする。

次回のPhase 4作業順:

1. ハンコン接続PCで最新commitをpullし、`oit_interfaces`と`oit`を再buildする。
2. Kobukiを停止したまま、通常の`WARNING / STEADY / 0.05`を1イベントだけ送る。
3. `pattern=1`のまま実機確認済み矩形波が出力され、CLEAR、watchdog、Ctrl+Cで停止することを確認する。
4. nodeの停止・再起動を3回行い、毎回effectを作成・停止でき、slot枯渇や多重writerがないことを確認する。
5. 0.05が弱い場合は、まず0.05のまま通知cadenceと有限継続時間を比較する。
6. cadence調整後も不足する場合だけ、0.08を次候補として別の安全gate、単発時間、中止条件を設計する。
...
通知cadence実機比較（2026-09-16）:

- `continuous / 0.05 / 0.5秒`は自動判定PASSで、危険通知として知覚できたが少し弱いという評価だった。
- `double / 0.05 / 0.5秒`は自動判定PASSだったが、2回の区切りは明瞭でなかった。
- `triple / 0.05 / 0.5秒`は自動判定PASSで、3回の振動として識別できた。
- watchdogはactive指令停止後約106 msで物理出力を停止し、意図した`fault=True`を報告した。
- CLEAR、shutdown、複数回のnode再起動で残留出力、slot枯渇、writer競合、異音、急回転、発熱は確認されなかった。
- Phase 5の採用候補を`triple / 0.05`とし、強度上限は変更しない。

Phase 5へ進む前の実装順（2026-09-16承認済み）:

1. 認識側publisherへ、実機確認した0.5秒・3連の有限cadenceを追加する。
2. WARNING開始時に1セットだけ開始し、WARNING_HOLDだけでは再発火させない。
3. CLEAR/PATHは途中でも即時停止し、CRITICALへの上昇時は新しい3連通知を開始する。
4. 継続WARNING中は無制限に再発火させず、再通知周期は別の実機評価まで追加しない。
5. 従来設定は`continuous`のまま互換性を維持し、日付付き設定で`triple`を明示する。
6. 単体試験、過去録画のdry-run、status照合を完了してからhardwareへ再接続する。
